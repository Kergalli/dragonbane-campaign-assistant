...
import { MODULE_ID, MODULE_NAME, SETTINGS, SOCKET_EVENTS } from './constants.js';
...
class DragonbaneCampaignAssistant {
  ...
  static onReady() {
    ...
    if (!game.modules.get('socketlib')?.active) {
      ...
      ui.notifications.error(Utils.localize('CAMPAIGN_ASSISTANT.errors.socketlibRequired'));
      return;
    }
  ...
  static setupSocket() {
    ...
    this.socket.register(SOCKET_EVENTS.RECORD_ADVANCEMENT, async (actorId, actorName, results) => {
  ...
  static async _recordAdvancementToJournal(actorId, actorName, results) {
    ...
    let folder = game.folders.find(f => f.name === 'Advancement History' && f.type === 'JournalEntry');
    ...
    let journal = game.journal.find(j => j.name === actorName && j.folder?.id === folder.id);
    ...
    const successList = results
      .filter(r => r.success)
      .map(r => {
        const maxLabel = r.reachedMaximum ? ' <strong>(Max Level 18)</strong>' : '';
    ...
    const newEntry = `
      ...
      ${heroicAbilitiesLine}
      ${successCount > 0 ? `<ul>${successList}</ul>` : '<p><em>No skills advanced this session.</em></p>'}
    `;
  ...
  static setupActorSheetButtons() {
    ...
    Hooks.on('renderDoDCharacterSheet', (sheet, html, data) => {
      ...
      setTimeout(() => {
        ...
        const appElement = sheet.element instanceof jQuery ? sheet.element[0] : sheet.element;
        ...
        headerButton.classList.add('header-control', 'icon', 'session-advancement-control');
        headerButton.type = 'button';
        headerButton.innerHTML = '<i class="fas fa-circle-arrow-up"></i>';
        headerButton.setAttribute('data-tooltip', Utils.localize('CAMPAIGN_ASSISTANT.buttons.sessionAdvancement'));
        headerButton.setAttribute('aria-label', Utils.localize('CAMPAIGN_ASSISTANT.buttons.sessionAdvancement'));
